...
function addJsExtensionToImports(content, filePath) {
  ...
  return content.replace(
    ...
    (match, prefix, quote, relativePath) => {
      ...
      if (relativePath.includes('.js') || relativePath.includes('.ts') || relativePath.includes('.json')) {
        return match
      }
...
function processFile(filePath) {
  ...
  if (content !== updatedContent) {
    ...
    if (content.includes("from './") || content.includes("from '../")) {
      console.log(`No changes made to: ${path.relative(ESM_DIR, filePath)} (has relative imports)`)
    }
  }
}
